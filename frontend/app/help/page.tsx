...
const SECTIONS: { title: string; href: string; items: [string, string][] }[] = [
  {
    ...
    items: [
      ...
      ["云端跟踪池", "feed/watchlist.json 的跟踪池(OpenClaw 每日全析对象),默认收起,▼ 展开为热力图"],
      ["语言切换", "右上「中 | EN」,330 条双语名称库,搜索框中英代码三路联想"],
      ["导航与快捷键", "每页顶部同一组入口(当前页高亮);键盘先按 g 再按 h/d/s/t/i/r/p/a/o/? 直达对应页"],
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      ["☯ 缠论买卖点统计", "全宇宙真回测:各类买卖点 +5/+20 根胜率与进行中信号清单(每周六重算)"],
    ],
  },
  {
  ...
  {
    title: "🤖 自动化(GitHub Actions 定时,你不用做任何事;时间均为 UTC)", href: "/desk/",
    items: [
      ["盘中 5 分钟机器报告", "工作日 00:00-20:00:Winter 循环在岗时由她推 live 分支;掉线则 Actions 自续链三轮循环 + 每小时看门狗兜底"],
      ["12:40 盘前要素包", "期货/亚欧/隔夜加密/昨日异动 → /desk 今日要点;13:40 与 19:45 看门狗核对 Winter 盘前/收盘前报告,缺投只维护一张滚动 Issue,补投自动关闭"],
      ["22:30 选股 · 01:00 个股解读", "标普500∪纳指100 全扫描 ≥80 入池,当日 Issue 只保留一张;次日 01:00 全池确定性解读(Actions 备胎,非 LLM)"],
      ["23:30 日报 Issue", "选股入库 /tracker、昨日表现、自选异动、缠论信号;旧日报自动关闭"],
      ["市场快照 / 缠论统计 / 13F", "工作日两次指数+FNG+BTC 历史;每周六缠论买卖点全宇宙统计;周一/四查 SA LP 新 13F"],
      ["每 2h Hyperliquid 快照", "永续资金费/OI/跨所错位、HIP-3 合成美股与 Pre-IPO 历史,周末照跑"],
      ["09:00 / 21:00 feed 审计", "实效性 + 连贯性审计 → /intel 健康面板与 feed/health.json;critical 开 Issue,恢复自动关闭"],
    ],
  },
];
