...
- `cryptodata.py` and `binancedata.py` also rewrite the tracked `crypto_universe.json` and `binance_universe.json` from live liquidity selection. `pit_membership.py` and `study_downshift.py` can rewrite tracked universe snapshots. Review those diffs rather than treating a refresh as disposable cache.
...
| [`binancedata.py`](binancedata.py) | Binance market and funding loader | Always uses live Binance mirror/ZIP endpoints; rewrites tracked universe and ignored caches | Cache producer; historical [`README_binance.md`](README_binance.md) | Current-volume/current-listed selection creates future-membership and survivorship bias; missing funding downloads can be skipped. |
...
| [`cryptodata.py`](cryptodata.py) | Hyperliquid candles and funding loader | Always uses live Hyperliquid APIs; rewrites tracked universe and ignored caches | Cache producer; historical [`README_crypto.md`](README_crypto.md) and [`README_crypto_pipeline.md`](README_crypto_pipeline.md) | Live universe selection and short, drifting history prevent byte-for-byte reproduction of old snapshots. |
...
| [`factor_factory.py`](factor_factory.py) | Deterministic formula-candidate factory with statistical and cost gates | Cache-only; workflow prefetches equity, benchmark, sector ETF, and PIT inputs | Stdout; optional feed report; no dedicated Markdown result | The breadth gate is explicitly unimplemented, the split is retrospective, and PIT data cannot restore delisted prices. |
...
```bash
python -c "import ast,pathlib; [ast.parse(p.read_text(encoding='utf-8')) for p in pathlib.Path('backtest').glob('*.py')]"
python -c "import json,pathlib; json.loads(pathlib.Path('backtest/results/backtest_report.json').read_text(encoding='utf-8'))"
```
