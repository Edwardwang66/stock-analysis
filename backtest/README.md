...
> **一个诚实、无未来函数的方向性因子,在美股大盘股上的"正确率"上限约 62~67%**(1 年期)。
> 想看到 85-90% 的数字,靠的不是预测力,而是**改变"正确"的定义**(止盈+不止损的路径标签),
> 那会换来"胜率高但单笔亏损巨大、负偏"的副作用。下面用真实回测把这件事拆给你看。
...
## 三、那 85-90% 怎么来?—— 拆穿这个数字(`barrier.py`)
...
python run.py                  # 主回测:多 horizon 正确率表
python experiment_regime.py    # 大盘择时闸门实验(看天花板)
python barrier.py              # 路径标签实验(看 85-90% 怎么来的)
```
