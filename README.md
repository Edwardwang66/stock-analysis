...
+ 🔮 Pre-IPO 永续定价(SpaceX/Anthropic/OpenAI);**🌙 夜盘报价层挂全站**——美股闭市时首页卡片/个股页
显示 Hyperliquid 24/7 永续价与隐含跳空(Cycle8 实证 corr 0.965);
...
**6-10 交互升级(学习 Hyperliquid 单标的工作台范式)**:**★ 自选行情条**挂全站顶部(加密 WS 实时/股票轮询,点击直达);
个股页**标题即标的切换器**(下拉=搜索+自选+热门,原地换标的不回首页);**一行统计条**(现价/涨跌/高低/昨收/量/52周位置/
盘前盘后/夜盘/**距开收盘倒计时**/**财报倒计时**,hover 出解释);**💼 我的信息上图**——持仓成本线+价格提醒线直接画在 K线上,
提醒线**按住拖拽改价**;分析模块收进**工作台 tabs**(按需加载),一键切**研报模式**全展开(模块可折叠、↑↓ 排序);
加密标的图表右侧 **Hyperliquid 式盘口**(Binance 真实 L2 深度条+逐笔成交 tape;股票免费源无深度,不估算冒充);
**偏好全持久化**(指标开关/周期/范围/tab/模块顺序);滚动时**吸顶迷你价格条**。
