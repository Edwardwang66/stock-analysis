...
> **Last verified commit:** `761dbc81494e9f2d3a21ba40fd83c474a864b6d3`
...
Feed watchdog has two schedules: general audit runs and hourly weekday intraday recovery windows. It attempts the live-branch intraday fallback only during weekday hours 00–20 UTC (the hours its own hourly cron and the intraday cron cover), commits main-branch health before touching issues, then opens a deduplicated `feed-stale` issue when the audit reports critical findings and closes that issue when a later audit is clean. If `gh issue list` fails, the issue step logs a warning and skips issue maintenance for that run.

OpenClaw watchdog checks for expected dated premarket and close Markdown. It diagnoses missing files, not their analytical correctness or completeness. It keeps at most one open `openclaw-miss` issue per kind, matching issues by the exact title prefix `⏰ OpenClaw <kind>报告缺投` (a plain substring match would merge the two kinds because `收盘前` contains `盘前`): a repeated miss updates that issue to the latest date, and an on-time file closes it. Bulk closes are paced at one issue per two seconds to stay under GitHub's secondary rate limit; the same pacing applies to the screener and digest cleanups.
